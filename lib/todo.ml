...
let due_date name =
  ...
  with Not_found -> "Item " ^ name ^ " was not found in todo list"

let notes name =
  try
    let itm = find name !todo in
    match itm.notes with None -> "No notes provided." | Some n -> n
  with Not_found -> "Item " ^ name ^ " was not found in todo list"
...
let update_more ?due_date ?notes ?category ?time ?duration ?priority ?complete
  ...
  with Not_found -> Printf.printf "Item %s was not found in todo list\n" name
...
module Basic = struct
  ...
  let order_bindings m =
    ...
    let l = bindings m in
    let cat_sort =
      sort (fun (_, i1) (_, i2) -> String.compare i1.category i2.category) l
    in
    let prio_sort =
      sort (fun (_, i1) (_, i2) -> Int.compare i2.priority i1.priority) cat_sort
    in
  ...
  let due_today () =
    ...
    let today_map =
      filter
        (fun _ i ->
          match i.due_date with None -> false | Some dd -> dd = today)
        !todo
    in
...
module Json = struct
  let string_list_to_json sl = List.map (fun s -> `String s) sl
  ...
  let display_all () =
    ...
    let p_sort =
      sort (fun (_, i1) (_, i2) -> Int.compare i1.priority i2.priority) b
    in
    let sorted =
      sort (fun (_, i1) (_, i2) -> cmp_date i1.due_date i2.due_date) p_sort
    in
    iter
      (fun (_, i) ->
        display i ;
